...
# ║                    🔐 CREDENCIAIS                           ║
# ╚══════════════════════════════════════════════════════════════╝
TELEGRAM_TOKEN   = "8619957600:AAHNnNvKGc5auf0-Cz3cij_GqCK-ZQELJmU"
TELEGRAM_CHAT_ID = "-1003793130834"

# ╔══════════════════════════════════════════════════════════════╗
# ║                 🌐 SCRAPERAPI (anti-Cloudflare)             ║
# ║  Cadastro gratuito: https://www.scraperapi.com              ║
# ║  5.000 requests/mês grátis no plano free                   ║
# ║  Cole sua chave abaixo depois de criar a conta             ║
# ╚══════════════════════════════════════════════════════════════╝
SCRAPERAPI_KEY  = "e288b07206ea405eb889c4fcd63331a6"
...
SHOWS = [
    {
        ...
        "setor_filtro": "pista premium",   # so alerta se esse setor estiver disponivel
    },
    {
        "data": "28/10/2026",
        "dia": "Quarta-feira",
        ...
        "setor_filtro": None,              # alerta para qualquer ingresso
    },
    {
        "data": "30/10/2026",
        "dia": "Sexta-feira",
...
# ╔══════════════════════════════════════════════════════════════╗
# ║         🌐 REQUEST COM SCRAPERAPI (anti-Cloudflare)         ║
# ╚══════════════════════════════════════════════════════════════╝
def fazer_request(url, timeout=30):
    """
    Faz GET na URL.
    - Se SCRAPERAPI_KEY estiver configurada: passa pela ScraperAPI (resolve Cloudflare/403).
    - Caso contrario: request direto normal (pode levar 403 do Ticketmaster).
    """
    if SCRAPERAPI_KEY:
        params = {
            "api_key": SCRAPERAPI_KEY,
            "url":     url,
            "render":  "false",   # sem JS (mais rapido e barato)
            "country_code": "br", # IP brasileiro
        }
        return requests.get(SCRAPERAPI_URL, params=params, timeout=timeout)
    else:
...
def _extrair_zona_compra(html):
    """Extrai somente a parte do HTML relevante para compra."""
    html_l = html.lower()
    # Tenta pegar o trecho ao redor das palavras-chave de compra
    alvo = ["buy", "cart", "ticket", "comprar", "ingresso", "setor", "preco", "price"]
    for palavra in alvo:
        idx = html_l.find(palavra)
        if idx != -1:
            return html[max(0, idx-200):idx+500]
    return html[:2000]  # fallback: primeiros 2000 chars
...
def pagina_mudou(show_data, html):
    ...
    zona         = _extrair_zona_compra(html)
    novo_parcial = hashlib.md5(zona.encode("utf-8", errors="ignore")).hexdigest()
...
def intervalo_por_horario(modo_alerta=False, modo_rapido=False):
    """Retorna o intervalo de espera em segundos — 2min fixo 24h."""
    if modo_rapido:
        return random.randint(20, 35)   # pos-mudanca: ~30s
    if modo_alerta:
        return random.randint(55, 75)   # zona de compra mudou: ~1min
    return random.randint(100, 130)     # padrao 24h: ~2min (dia E madrugada)
...
def extrair_detalhes(html):
    ...
    # ── Tipo de ingresso ─────────────────────────────────────
    tipos = []
    for tipo in ["inteira", "meia", "half price", "full price", "student", "estudante", "social"]:
...
def verificar_show(show):
    ...
    for tentativa in range(tentativas):
        try:
            ...
            r          = fazer_request(show["url"], timeout=30)
            html       = r.text
            html_lower = html.lower()
            ...
            # Evita falsos "ZONA MUDOU" em respostas de erro (403, etc)
            ...
            if r.status_code == 403:
                print(f"     Bloqueado (403) - tentativa {tentativa+1}/{tentativas}")
                if not SCRAPERAPI_KEY:
                    print(f"     DICA: Configure SCRAPERAPI_KEY para resolver bloqueios do Cloudflare.")
                registrar_erro_show(show["data"])
                if tentativa == tentativas - 1:
                    print(f"     Todas as tentativas bloqueadas (403). Ticketmaster bloqueando acesso.")
            ...
            # Disponivel
            palavras_disponivel = [
                # Portugues - acoes de compra
                "comprar ingresso", "compre agora", "adicionar ao carrinho",
                "selecionar ingresso", "escolher ingresso", "ingressos disponiveis",
                "selecione o setor", "escolha o setor", "quantidade de ingressos",
                "valor do ingresso", "finalizar compra", "ir para o carrinho",
                "comprar agora", "ingresso disponivel", "ingressos a partir",
                "selecione a quantidade", "escolha seu ingresso", "mapa de assentos",
                # Ingles - acoes de compra
                "buy ticket", "buy now", "add to cart", "select tickets",
                "purchase", "get tickets", "book now", "available tickets",
                "choose seats", "select seats", "ticket quantity",
                "checkout", "proceed to checkout", "tickets available",
                "add tickets", "select ticket type", "ticket type",
                # Sinais de JSON/JS no HTML (Ticketmaster global)
                "ticketid", "seatmap", "pricezone", "offercode",
                "inventorytype", "addtocart", "buybutton",
                "ticket_price", "ticket_availability", "on_sale",
                "onsale", "salestatus", "sale_status",
                # Sinais extras Ticketmaster BR
                "pista premium", "pista vip", "cadeira numerada",
                "area vip", "ingresso inteira", "ingresso meia",
                "setor disponivel", "places available", "availability",
                "quantity_available", "tickets_remaining", "em estoque",
                "em venda", "venda aberta", "venda disponivel",
                '"available":true', '"status":"available"',
                '"onSale":true', '"isAvailable":true',
                '"inStock":true', '"inventory":', '"quantity":',
            ]
            ...
            if disponivel and not esgotado:
                return "disponivel", extrair_detalhes(html), mudou_zona
...
def monitorar():
    ...
    msg_inicio = (
        f"============================\n"
        f"  BOT BTS ARMY ONLINE!\n"
        f"============================\n\n"
        f"Iniciado: {hora_inicio.strftime('%d/%m/%Y as %H:%M:%S')}\n\n"
        f"----------------------------\n"
        f"O QUE EU MONITORO:\n"
        f"----------------------------\n"
        f"THE ROSE\n"
        f"21/06/2026 - Domingo (so alerta se Pista Premium disponivel)\n\n"
        f"BTS WORLD TOUR - ARIRANG\n"
        f"28/10/2026 - Quarta-feira\n"
        f"30/10/2026 - Sexta-feira\n"
        f"31/10/2026 - Sabado\n\n"
        f"----------------------------\n"
        f"COMO EU FUNCIONO:\n"
        f"----------------------------\n"
        f"Verifico a cada ~2 minutos, 24 horas por dia, inclusive de madrugada. Nenhum ingresso passa!\n\n"
        f"Modo de acesso: {'ScraperAPI ATIVA (anti-Cloudflare)' if SCRAPERAPI_KEY else 'Acesso direto (sem ScraperAPI)'}\n\n"
        f"Primeiro tento a API interna do Ticketmaster (mais rapido). Se nao responder, raspo o HTML da pagina.\n\n"
        f"Se a zona de compra do site mudar, entro em modo alerta e verifico a cada ~1 minuto.\n\n"
        f"Se confirmar ingresso disponivel, verifico 5x seguidas em 2 minutos pra garantir.\n\n"
        f"----------------------------\n"
        f"QUANDO ABRIR INGRESSO:\n"
        f"----------------------------\n"
        f"Mando 3 alertas seguidos com:\n"
        f"- Dia e data do show\n"
        f"- Setores disponiveis\n"
        f"- Precos detectados\n"
        f"- Tipo de ingresso (inteira/meia)\n"
        f"- Link direto para comprar\n\n"
        f"----------------------------\n"
        f"OUTROS ALERTAS:\n"
        f"----------------------------\n"
        f"- Fila virtual detectada: aviso 3x para entrar na fila\n"
        f"- Zona de compra mudou: aviso imediato de pre-abertura\n"
        f"- Ingresso esgotado voltou: aviso de devolucao detectada\n"
        f"- A cada 12h: confirmacao de que estou rodando\n"
        f"- 10+ erros seguidos: aviso de possivel bloqueio\n\n"
        f"----------------------------\n"
        f"PROTECOES ANTI-BLOQUEIO:\n"
        f"----------------------------\n"
        f"- 14 navegadores diferentes rotativos\n"
        f"- Headers e idiomas variados a cada requisicao\n"
        f"- 4 tentativas por verificacao com espera progressiva\n"
        f"- Backoff automatico por show com muitos erros\n"
        f"- Reconexao automatica se cair a internet\n"
        f"- Reinicio automatico se travar\n\n"
        f"============================\n"
        f"ARMY FIGHTING! BORAHAE!\n"
        f"Nenhum ingresso me escapa!\n"
        f"============================"
    )
    ...
    while True:
        try:
            ...
            if horas_desde_12h >= 12:
                # Alterna entre frases BTS e The Rose
                if random.random() < 0.5:
                    _, frase_motivacional = random.choice(FRASES_BTS)
                    musica, mensagem_musica = random.choice(MUSICAS_BTS)
                else:
                    frase_motivacional = random.choice(FRASES_THE_ROSE)
                    musica, mensagem_musica = random.choice(MUSICAS_THE_ROSE)

                # Monta status real de cada show
                linhas_status = []
                for s in SHOWS:
                    st = status_anterior.get(s["data"], "aguardando")
                    emoji = "⏳" if st in ("nao_disponivel", "nao_abriu", "aguardando") else \
                            "✅" if st == "disponivel" else \
                            "❌" if st == "esgotado" else \
                            "🟡" if st == "fila" else "⚠️"
                    filtro_txt = f" (so Pista Premium)" if s.get("setor_filtro") else ""
                    linhas_status.append(f"{emoji} {s['artista']} {s['data']}{filtro_txt}: {st}")
                status_txt = "\n".join(linhas_status)

                enviar_telegram(
                    f"Bot rodando ha {horas_rodando:.0f}h | {total_verif} verificacoes feitas\n\n"
                    f"STATUS ATUAL:\n{status_txt}\n\n"
                    f"Musica do momento: {musica}\n"
                    f"{mensagem_musica}\n\n"
                    f"{frase_motivacional}"
                )
            ...
            for show in SHOWS:
                ...
                if mudou_zona and status not in ("disponivel", "fila"):
                    ...
                    print(f"   ZONA DE COMPRA MUDOU! Entrando em modo rapido...")
                    enviar_telegram(
                        f"⚠️ Zona de compra do show {data} mudou!\n\n"
                        f"{artista} - {show['dia']} - {data}\n"
                        f"Area de venda foi atualizada - pode ser pre-abertura!\n"
                        f"Verificando em modo rapido agora...\n\n"
                        f"Link: {show['url']}"
                    )
                ...
                if status_anterior.get(data) == "esgotado" and status == "disponivel":
                    enviar_telegram(
                        f"🔄 INGRESSO VOLTOU! Devolucao detectada!\n\n"
                        f"{artista} - {show['dia']} - {data}\n"
                        f"Estava esgotado e voltou a ficar disponivel!\n\n"
                        f"Link: {show['url']}"
                    )
                ...
                if status == "disponivel":
                    ...
                    # ── Filtro de setor (ex: The Rose so alerta se tiver Pista Premium) ──
                    if filtro:
                        html_raw = detalhes.get("_html_raw", "")
                        setor_encontrado = (
                            any(filtro.lower() in s.lower() for s in setores) or
                            filtro.lower() in str(det.get("sinais", [])).lower() or
                            filtro.lower() in html_raw.lower()
                        )
                        if not setor_encontrado:
                            print(f"   Disponivel mas setor '{filtro}' NAO detectado - aguardando setor especifico.")
                            continue  # nao notifica, continua monitorando

                    setores_txt = "\n".join([f"  • {s}" for s in setores]) or "  • Verificar no site"
                    precos_txt  = "  " + " | ".join(det.get("precos", [])) if det.get("precos") else "  Verificar no site"
                    tipos_txt   = "  " + " | ".join(det.get("tipos", []))  if det.get("tipos")  else "  Verificar no site"
                    sinais_txt  = "\n".join([f"  ✓ {s}" for s in det.get("sinais", [])]) or "  Pagina com sinais de venda ativa"

                    # Cabecalho do alerta varia por artista
                    if artista == "BTS":
                        cabecalho = "BTS WORLD TOUR - ARIRANG"
                        rodape    = "ARMY FIGHTING! BORAHAE!"
                    else:
                        cabecalho = f"{artista.upper()} - SAO PAULO"
                        rodape    = "CORRE! INGRESSOS ESGOTAM RAPIDO!"
                        if filtro:
                            rodape = f"PISTA PREMIUM DISPONIVEL!\n{rodape}"

                    alerta = (
                        f"🚨🚨🚨 CORRE! INGRESSO ABRIU! 🚨🚨🚨\n\n"
                        f"{cabecalho}\n"
                        f"============================\n"
                        f"DIA: {show['dia'].upper()}\n"
                        f"DATA: {data}\n"
                        f"Detectado: {agora.strftime('%d/%m/%Y as %H:%M:%S')}\n"
                        f"============================\n\n"
                        f"SETORES DETECTADOS:\n{setores_txt}\n\n"
                        f"PRECOS DETECTADOS:\n{precos_txt}\n\n"
                        f"TIPOS DE INGRESSO:\n{tipos_txt}\n\n"
                        f"O QUE O BOT ENCONTROU:\n{sinais_txt}\n\n"
                        f"============================\n"
                        f"LINK DIRETO PARA COMPRAR:\n"
                        f"{show['url']}\n"
                        f"============================\n\n"
                        f"ABRE O LINK AGORA!\n"
                        f"Ingressos esgotam em minutos!\n"
                        f"{rodape}"
                    )
                    for i in range(3):
                    ...
                    print(f"   DISPONIVEL! Alerta completo enviado 3x!")

                    # ── Confirmacao em rajada: bate 5x nos proximos 2min ──
                    print(f"   Iniciando confirmacao em rajada...")
                    for conf in range(5):
                        time.sleep(25)
                        st2, det2, _ = verificar_show(show)
                        if st2 == "disponivel":
                            enviar_telegram(
                                f"✅ CONFIRMADO #{conf+1}/5 - AINDA ABERTO!\n"
                                f"{show['dia']} {data}\n"
                                f"Link: {show['url']}"
                            )
                        else:
                            enviar_telegram(
                                f"⚠️ Confirmacao #{conf+1}/5: status mudou para '{st2}'\n"
                                f"Pode ter esgotado rapido. Tente mesmo assim!\n"
                                f"Link: {show['url']}"
                            )
                            break
                    ...
                    erros_consecutivos = 0
                    alerta_fila = (
                        f"SALA DE ESPERA ATIVADA!\n\n"
                        f"{artista} - {show['dia']}, {data}\n"
                        f"Detectado: {agora.strftime('%d/%m/%Y %H:%M:%S')}\n\n"
                        f"FILA VIRTUAL DETECTADA!\n"
                        f"Isso significa que a venda pode estar\n"
                        f"ABRINDO AGORA MESMO!\n\n"
                        f"ENTRE NA FILA AGORA:\n"
                        f"{show['url']}\n\n"
                        f"CADA SEGUNDO CONTA!\n"
                        f"Abra o link e entre na fila!"
                    )
                    for i in range(3):
                    ...
                    if erros_consecutivos >= 10:
                        enviar_telegram(
                            f"ATENCAO - {erros_consecutivos} erros consecutivos!\n\n"
                            f"O bot esta com dificuldade de acessar o Ticketmaster.\n"
                            f"Pode ser bloqueio temporario.\n\n"
                            f"Continuando tentativas...\n"
                            f"Horario: {agora.strftime('%d/%m %H:%M')}"
                        )
                        erros_consecutivos = 0
                else:
                    erros_consecutivos = 0
            ...
            if em_modo_rapido:
                ...
                modo_str = "pico" if 10 <= hora_atual <= 22 else "madrugada"
...
def main():
    ...
    while True:
        try:
            ...
            try:
                enviar_telegram(
                    f"Bot se reiniciando automaticamente!\n"
