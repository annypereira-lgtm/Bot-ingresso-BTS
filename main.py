...
# ╚══════════════════════════════════════════════════════════════╝
_ultimo_request_ts = [0.0]  # controle de delay global entre requests

def _delay_humano():
    """Espera um tempo aleatorio que imita comportamento humano entre requests."""
    agora = time.time()
    desde_ultimo = agora - _ultimo_request_ts[0]
    # Garante pelo menos 2-4s entre requests diretos
    minimo = random.uniform(2.0, 4.5)
    if desde_ultimo < minimo:
        time.sleep(minimo - desde_ultimo + random.uniform(0.1, 0.8))
    _ultimo_request_ts[0] = time.time()
...
def fazer_request(url, timeout=30, forcar_scraper=False, usar_mobile=False):
    ...
    if forcar_scraper and get_scraper_key():
        params = {
            ...
            "url":          url,
            "render":       "true",       # executa JavaScript
            "country_code": "br",         # IP brasileiro
            "device_type":  "desktop",    # simula desktop
            "premium":      "true",       # usa proxies premium (menos bloqueados)
            "session_number": str(random.randint(1, 5)),  # simula sessao continua
        }
        print(f"     [ScraperAPI] Usando proxy premium BR...", end="")
        try:
            ...
            print(f" FALHOU ({e}), tentando direto...")
            # Se ScraperAPI falhar, tenta direto com headers mobile
            return requests.get(url, headers=get_headers(mobile=True), timeout=timeout, allow_redirects=True)

    # Request direto — alterna entre desktop e mobile
    mobile = usar_mobile or random.random() < 0.3  # 30% das vezes usa mobile
...
_status_confirmado   = {}   # ultimo status confirmado por 2 leituras consecutivas
    ...
    for tentativa in range(tentativas):
        try:
            ...
            if disponivel:
                ...
                fp_anterior = _status_confirmado.get(show["data"], "")
                ...
                _status_confirmado[show["data"]] = fp_atual
        ...
        try:
            ...
            for show in SHOWS:
                ...
                if status == "disponivel":
                    ...
                    # Cooldown de fila: evita spam (2 min entre alertas de fila)
                    if not hasattr(monitorar, '_ultimo_alerta_fila'):
                        monitorar._ultimo_alerta_fila = {}
                    agora_ts = time.time()
                    ultimo_fila = monitorar._ultimo_alerta_fila.get(data, 0)
                    if agora_ts - ultimo_fila < 120:
                        print(f"   Fila detectada mas cooldown de fila ativo — sem spam.")
                    else:
                        if artista == "BTS":
                            alerta_fila = (
                                f"🟡 FILA VIRTUAL ATIVA — BTS!\n"
                                f"💜 BTS WORLD TOUR ARIRANG — SAO PAULO\n"
                                f"📅 {show['dia'].upper()} — {data}\n"
                                f"🕐 {agora.strftime('%d/%m/%Y %H:%M:%S')}\n\n"
                                f"⚠️ O Ticketmaster abriu fila virtual!\n"
                                f"Isso significa que a venda pode estar\n"
                                f"abrindo AGORA ou em instantes!\n\n"
                                f"👉 ENTRE NA FILA JA:\n{show['url']}\n\n"
                                f"🏃 NAO PERDE TEMPO! ARMY FIGHTING! 💜"
                            )
                        else:
                            alerta_fila = (
                                f"🟡 FILA VIRTUAL ATIVA — THE ROSE!\n"
                                f"🌹 THE ROSE TOUR 2026 [ROSETOPIA] — SAO PAULO\n"
                                f"📅 {show['dia'].upper()} — {data}\n"
                                f"🕐 {agora.strftime('%d/%m/%Y %H:%M:%S')}\n\n"
                                f"⚠️ O Ticketmaster abriu fila virtual!\n"
                                f"Pode ser que a Pista Premium + Meia\n"
                                f"esteja abrindo AGORA ou em instantes!\n\n"
                                f"👉 ENTRE NA FILA JA:\n{show['url']}\n\n"
                                f"🏃 CORRE! NAO PERDE ESSA CHANCE!"
                            )
                        for i in range(3):
                            ...
                            if i < 2:
                                time.sleep(5)
                        monitorar._ultimo_alerta_fila[data] = agora_ts
