    ...
    for tentativa in range(tentativas):
        try:
            ...
            if disponivel and not esgotado:
                ...
                # ── Barreira 1: exige mudanca real na zona de compra ──
                # Se a zona de compra nao mudou desde a ultima leitura,
                # e porque a pagina esta estatica — nao houve abertura real.
                # Excecao: primeira vez que vemos a pagina (mudou_zona pode
                # ser False por ser a leitura inicial) — nesse caso deixa
                # passar para o Groq decidir.
                ja_tinha_hash = show["data"] in _html_hashes and \
                                _html_hashes_parcial.get(show["data"]) is not None
                if ja_tinha_hash and not mudou_zona:
                    fp_atual = _extrair_fingerprint_status(html)
                    fp_anterior = _status_confirmado.get(show["data"], "")
                    if fp_atual == fp_anterior:
                        print(f"     Palavras detectadas mas zona de compra nao mudou — ignorando (pagina estatica).")
                        resetar_erro_show(show["data"])
                        return "nao_disponivel", [], mudou_zona
                    else:
                        print(f"     Fingerprint de status mudou! Prosseguindo verificacao...")
                else:
                    print(f"     Primeira leitura ou zona mudou — prosseguindo verificacao...")

                # ── Barreira 2: confirmacao pelo Groq ─────────────────
                print(f"     Confirmando com Groq...")
                ...
                # Salva fingerprint do status atual para comparacao futura
                _status_confirmado[show["data"]] = _extrair_fingerprint_status(html)
    ...
    while True:
        try:
            ...
            for show in SHOWS:
                ...
                if anterior in ("esgotado", "nao_disponivel", "nao_abriu") and status == "disponivel":
                    print(f"   STATUS MUDOU: {anterior} -> disponivel! Processando alerta...")

                status_anterior[data] = status

                # ── CONFIRMACAO DUPLA ANTI-FALSO-POSITIVO ──────────
                # So prossegue se o status aparecer em 2 leituras consecutivas
                # E for diferente do ultimo status confirmado
                if status in ("disponivel", "fila"):
                    if not status_realmente_mudou(data, status):
                        print(f"   [AntiSpam] Aguardando 2a confirmacao de '{status}'...")
                        continue
                else:
                    # Para status nao-criticos, atualiza confirmado sem bloquear
                    status_realmente_mudou(data, status)
