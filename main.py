...
# ╚══════════════════════════════════════════════════════════════╝
USER_AGENTS = [
    # Chrome Windows — mais comuns no Brasil
    ...
    # Mobile — Android Chrome (muito comum no Brasil)
...
# User agents especificos para API movel do Ticketmaster
USER_AGENTS_MOBILE_APP = [
    "Ticketmaster/1.0 (iPhone; iOS 17.4.1; Scale/3.00)",
    "Ticketmaster/2024.4.0 (Android 14; Pixel 8)",
    "TM-Android/2024.4 (Linux; Android 14; Build/UD1A.231105.004)",
]

# Session cookies simulados — parecem usuarios reais que ja visitaram o site
_session_pool = [
    {"tmps": f"tmps_{random.randint(100000,999999)}", "eps_sid": f"eps_{random.randint(10000,99999)}"},
    {"tmps": f"tmps_{random.randint(100000,999999)}", "eps_sid": f"eps_{random.randint(10000,99999)}"},
    {"tmps": f"tmps_{random.randint(100000,999999)}", "eps_sid": f"eps_{random.randint(10000,99999)}"},
]

def _gerar_cookie_str():
    """Gera string de cookie simulando sessao de usuario real."""
    sess = random.choice(_session_pool)
    tm_sid = f"TM_SID_{random.randint(1000000, 9999999)}"
    return f"tmps={sess['tmps']}; eps_sid={sess['eps_sid']}; TM_SESSION={tm_sid}"

def get_headers(mobile=False):
    """Headers realistas. mobile=True usa UA de celular (menos bloqueado)."""
    if mobile:
        ua = random.choice([ua for ua in USER_AGENTS if "Android" in ua or "iPhone" in ua])
    else:
    ...
    referers = [
        ...
        "https://www.ticketmaster.com.br/shows",
        "https://www.google.com.br/search?q=ticketmaster+ingressos",
        "https://www.google.com/search?q=bts+ingresso+sao+paulo",
    ]

    # Varia o viewport hint (indica resolucao da tela do usuario)
    viewports = ["1920,1080", "1366,768", "1440,900", "2560,1440", "390,844"]
    ...
    headers = {
        ...
        "Sec-Ch-Ua-Platform": '"Android"' if mobile else random.choice(['"Windows"', '"macOS"']),
        ...
        "Cookie": _gerar_cookie_str(),
    }

    # Adiciona viewport hint ocasionalmente (mais humano)
...
# ╚══════════════════════════════════════════════════════════════╝
# Delay por thread — cada show tem seu proprio contador, sem bloquear os outros
_ultimo_request_ts = {}   # show_url -> timestamp (por show, nao global)
_delay_lock = threading.Lock()

def _delay_humano(url=None):
    """Delay humano por URL/show — nao bloqueia threads de outros shows."""
    chave = url or "global"
    with _delay_lock:
        ultimo = _ultimo_request_ts.get(chave, 0.0)
        agora  = time.time()
        desde_ultimo = agora - ultimo
        minimo = random.uniform(2.0, 4.5)
        espera = max(0, minimo - desde_ultimo + random.uniform(0.1, 0.5))
        _ultimo_request_ts[chave] = agora + espera
    if espera > 0:
        time.sleep(espera)

# Flag global: desativa ScraperAPI automaticamente se "Host not in allowlist"
_scraperapi_desativada = [False]

def fazer_request(url, timeout=30, forcar_scraper=False, usar_mobile=False):
    """
    Faz GET na URL com estrategias anti-bloqueio:
    - Delay humano por URL (nao bloqueia outras threads)
    - Headers realistas com cookies simulados
    - ScraperAPI com parametros premium quando necessario
    - Fallback automatico para ScraperAPI reserva
    - Auto-desativacao da ScraperAPI se "Host not in allowlist"
    """
    _delay_humano(url)

    # Tenta ScraperAPI apenas se nao estiver desativada por erro de allowlist
    if forcar_scraper and get_scraper_key() and not _scraperapi_desativada[0]:
        # Tenta primeiro SEM render=true (mais rapido, menos JS overhead)
        # Se falhar com 403, tenta com render=true como fallback
        for usar_render in [False, True]:
            params = {
                "api_key":        get_scraper_key(),
                "url":            url,
                "country_code":   "br",
                "device_type":    "mobile" if usar_mobile else "desktop",
                "premium":        "true",
                "session_number": str(random.randint(1, 5)),
            }
            if usar_render:
                params["render"] = "true"

            modo_txt = "render+JS" if usar_render else "sem JS"
            print(f"     [ScraperAPI] Proxy BR ({modo_txt})...", end="")
            try:
                r = requests.get(SCRAPERAPI_URL, params=params, timeout=90)

                # Detecta erro de allowlist — desativa ScraperAPI e avisa uma vez
                if r.status_code == 403 and "allowlist" in r.text.lower():
                    _scraperapi_desativada[0] = True
                    print(f"\n     [ScraperAPI] HOST NAO PERMITIDO — desativando ScraperAPI.")
                    try:
                        enviar_telegram(
                            "\u26a0\ufe0f ScraperAPI bloqueada!\n"
                            "Erro: Host not in allowlist\n\n"
                            "O Railway nao esta na whitelist da sua conta.\n"
                            "Acesse: https://dashboard.scraperapi.com\n"
                            "\u2192 Settings \u2192 Allowed Hosts\n"
                            "\u2192 Adicione o dominio do Railway ou desative a restricao.\n\n"
                            "Continuando com requests diretos."
                        )
                    except Exception:
                        pass
                    break  # sai do loop, vai pro fallback direto
                ...
                # 403 sem allowlist e ainda nao tentou render — tenta com JS
                if r.status_code == 403 and not usar_render:
                    print(f" HTTP {r.status_code} — tentando com JS...", end="")
                    continue

                return r  # outro status — devolve para o caller tratar

            except Exception as e:
                print(f" FALHOU ({e})")
                break

    # Fallback direto (sem proxy) — alterna desktop/mobile
    mobile = usar_mobile or random.random() < 0.3
    return requests.get(url, headers=get_headers(mobile=mobile), timeout=timeout, allow_redirects=True)


def fazer_request_api_movel(url, timeout=20):
    """
    Request simulando o app oficial do Ticketmaster no celular.
    Endpoint diferente, menos sujeito a bloqueio que o site web.
    """
    _delay_humano(url)
    headers = {
        "User-Agent":     random.choice(USER_AGENTS_MOBILE_APP),
        "Accept":         "application/json",
        "Accept-Language":"pt-BR,pt;q=0.9",
        "Accept-Encoding":"gzip, deflate, br",
        "Connection":     "keep-alive",
        "X-Api-Key":      "b462oi7fic6pehcdkzony5bxhe",  # chave publica do app
        "X-Platform":     "iOS",
        "Referer":        "https://www.ticketmaster.com.br/",
    }
    try:
...
_status_confirmado       = {}   # ultimo status confirmado (str: "disponivel", "esgotado", etc.)
_fingerprints_confirmados= {}   # fingerprint da ultima leitura confirmada como disponivel
_status_candidato        = {}   # status candidato aguardando confirmacao
_contagem_candidato      = {}   # quantas vezes consecutivas esse candidato apareceu
...
def _extrair_fingerprint_status(html):
    """
    Extrai um fingerprint preciso do STATUS atual da pagina.
    Diferente de _extrair_sinais_relevantes — aqui o objetivo e
    criar uma assinatura unica que muda SOMENTE quando o status real muda.
    Ignora timestamps, contadores e outros elementos dinamicos que nao
    indicam mudanca de disponibilidade.
    """
    html_l = html.lower()
    sinais = []

    # ── Sinais de COMPRA ABERTA ───────────────────────────────
    compra_aberta = [
        "comprar ingresso", "add to cart", "addtocart", "buy now",
        "buybutton", "select tickets", "finalizar compra",
        "selecione o setor", "selecione a quantidade",
        '"available":true', '"onsale":true', '"on_sale":true',
        '"isavailable":true', '"instock":true',
        "offercode", "inventorytype", "ticketid", "pricezone",
    ]
    for s in compra_aberta:
        if s in html_l:
            sinais.append(f"COMPRA:{s}")

    # ── Sinais de ESGOTADO ────────────────────────────────────
    esgotado_sinais = [
        "esgotado", "sold out", "soldout", '"soldout":true',
        '"sold_out":true', '"available":false', "no tickets available",
        "todos os setores esgotados",
    ]
    for s in esgotado_sinais:
        if s in html_l:
            sinais.append(f"ESGOTADO:{s}")

    # ── Sinais de FILA ────────────────────────────────────────
    fila_sinais = ["waiting room", "fila virtual", "queue", "you are in line"]
    for s in fila_sinais:
        if s in html_l:
            sinais.append(f"FILA:{s}")

    # ── Precos detectados (ordenados para ser deterministicos) ─
    precos = sorted(set(re.findall(r'r\$\s*[\d.,]+', html_l)))[:5]
    if precos:
        sinais.append(f"PRECOS:{'|'.join(precos)}")

    # Retorna fingerprint ordenado e deterministico
    return "|".join(sorted(sinais))
...
def pagina_mudou(show_data, html):
    ...
    mudou_zona   = novo_parcial != anterior_parcial
    return mudou_total, mudou_zona


def status_realmente_mudou(show_data, novo_status):
    """
    Sistema de confirmacao dupla anti-falso-positivo.
    So retorna True (mudou de verdade) quando o novo status aparece
    em 2 leituras consecutivas E e diferente do ultimo status confirmado.

    EXCECAO CRITICA: se o status anterior confirmado era 'esgotado' (ou
    'nao_disponivel'/'nao_abriu') e o novo e 'disponivel', confirma na
    1a leitura — porque reposicoes sao rapidas e a dupla-confirmacao
    perderia a janela de compra.

    Fluxo normal:
    - 1a vez com novo status  → registra como candidato, retorna False
    - 2a vez com mesmo status → confirma, retorna True SE diferente do confirmado
    - Status diferente antes de confirmar → reseta candidato
    """
    confirmado  = _status_confirmado.get(show_data)
    candidato   = _status_candidato.get(show_data)
    contagem    = _contagem_candidato.get(show_data, 0)

    # ── EXCECAO: esgotado/indisponivel → disponivel: confirma imediatamente ──
    if novo_status == "disponivel" and confirmado in ("esgotado", "nao_disponivel", "nao_abriu", None):
        _status_confirmado[show_data]  = novo_status
        _status_candidato[show_data]   = novo_status
        _contagem_candidato[show_data] = 2
        print(f"     [AntiSpam] REPOSICAO detectada! '{confirmado}' → 'disponivel' — confirmando SEM esperar 2a leitura!")
        return True

    if novo_status == candidato:
        contagem += 1
        _contagem_candidato[show_data] = contagem
        if contagem >= 2 and novo_status != confirmado:
            # Confirmado por 2 leituras consecutivas e diferente do anterior
            _status_confirmado[show_data]  = novo_status
            _status_candidato[show_data]   = novo_status
            _contagem_candidato[show_data] = 2
            print(f"     [AntiSpam] Status CONFIRMADO por 2 leituras: {confirmado} → {novo_status}")
            return True
        return False
    else:
        # Novo candidato — reseta contagem
        _status_candidato[show_data]   = novo_status
        _contagem_candidato[show_data] = 1
        if novo_status != confirmado:
            print(f"     [AntiSpam] Novo candidato '{novo_status}' (aguardando 2a confirmacao)...")
        return False
...
# ╔══════════════════════════════════════════════════════════════╗
# ║         🔗 API TICKETMASTER DISCOVERY (publica)             ║
# ╚══════════════════════════════════════════════════════════════╝
# Mapa de slugs conhecidos para IDs reais da API Discovery
SLUG_PARA_ID = {
    "the-rose-sao-paulo":                       None,  # sera descoberto dinamicamente
    "venda-geral-bts-world-tour-arirang-28-10": None,
    "venda-geral-bts-world-tour-arirang-30-10": None,
    "venda-geral-bts-world-tour-arirang-31-10": None,
}
_cache_event_id = {}  # slug -> event_id descoberto

def _buscar_event_id_discovery(slug, artista):
    """Busca o event ID real na API Discovery do Ticketmaster usando o nome do artista."""
    if slug in _cache_event_id:
        return _cache_event_id[slug]
    try:
        # API Discovery publica — nao precisa de chave para consultas basicas
        params = {
            "keyword": artista,
            "countryCode": "BR",
            "size": 5,
        ...
        if r.status_code == 200:
            dados = r.json()
            eventos = dados.get("_embedded", {}).get("events", [])
            for ev in eventos:
                nome = ev.get("name", "").lower()
                if artista.lower() in nome or any(w in nome for w in artista.lower().split()):
                    eid = ev.get("id")
                    if eid:
                        _cache_event_id[slug] = eid
                        print(f"     [Discovery] Event ID encontrado: {eid} para '{ev.get('name')}'")
                        return eid
    except Exception as e:
        print(f"     [Discovery] Erro ao buscar ID: {e}")
    return None

def verificar_api_interna(show):
    """
    Tenta chamar endpoints JSON do Ticketmaster.
    Prioridade: API Discovery publica > endpoints internos > None (usa HTML).
    """
    slug    = show["url"].rstrip("/").split("/")[-1]
    artista = show.get("artista", "")

    # ── 1. Tenta API Discovery publica ───────────────────────
    event_id = _buscar_event_id_discovery(slug, artista)
    if event_id:
        try:
            headers = {
                "User-Agent": random.choice(USER_AGENTS),
            ...
            if r.status_code == 200:
                ...
                status_str = ev.get("dates", {}).get("status", {}).get("code", "")
                start_sale = sales.get("startDateTime", "")
                end_sale   = sales.get("endDateTime", "")
                print(f"     [Discovery] status={status_str} | venda={start_sale} ate {end_sale}")

                if status_str in ("onsale", "rescheduled"):
                    # Extrai precos e setores da API
                    price_ranges = ev.get("priceRanges", [])
                    precos = [f"R${p.get('min',0):.0f}-{p.get('max',0):.0f}" for p in price_ranges]
                    return "disponivel", {
                        "sinais": [f"API Discovery: status={status_str}"],
                        "setores": [],
                        "precos": precos,
                        "tipos": [],
                        "_html_raw": str(ev),
                    }
                elif status_str in ("offsale", "cancelled", "postponed"):
                    return "esgotado", []
                else:
                    return "nao_disponivel", []
        except Exception as e:
            print(f"     [Discovery] Erro ao checar evento: {e}")

    # ── 2. Fallback: endpoints internos do site BR ────────────
    endpoints = [
            ...
            if r.status_code == 200:
                try:
                    ...
                    if disponivel and not esgotado:
                        return "disponivel", {"sinais": ["API interna confirmou"], "setores": [], "precos": [], "tipos": [], "_html_raw": str(data)}
                    elif esgotado:
                        return "esgotado", []
                    else:
                        return "nao_disponivel", []
    ...
    return None, None  # nada funcionou, usa HTML normal


def _verificar_via_api_movel(show):
    """
    Tenta verificar disponibilidade via endpoint da API movel do Ticketmaster.
    Menos bloqueado que o site web normal.
    """
    slug = show["url"].rstrip("/").split("/")[-1]
    artista = show.get("artista", "")

    endpoints_movel = [
        f"https://api.ticketmaster.com/discovery/v2/events.json?keyword={artista}&countryCode=BR&size=3",
        f"https://www.ticketmaster.com.br/api/mobile/v2/event/{slug}",
        f"https://www.ticketmaster.com.br/api/mobile/event/{slug}/offers",
    ]

    for endpoint in endpoints_movel:
        r = fazer_request_api_movel(endpoint)
        if r is None or r.status_code != 200:
            continue
        try:
            data = r.json()
            data_str = str(data).lower()
            print(f"     [API Movel] respondeu: {endpoint}")
            disponivel = any(p in data_str for p in ["onsale", "available", "on_sale", "buy"])
            esgotado   = any(p in data_str for p in ["offsale", "sold_out", "soldout", "cancelled"])
            if disponivel and not esgotado:
                return "disponivel", {"sinais": ["API movel confirmou"], "setores": [], "precos": [], "tipos": [], "_html_raw": str(data)}
            elif esgotado:
                return "esgotado", []
        except Exception:
            pass
...
def verificar_show(show):
    ...
    # ── 1. Tenta API Discovery/interna (mais rapido) ──────────
    ...
    # ── 2. Tenta API movel (menos bloqueada que o site) ───────
    status_movel, det_movel = _verificar_via_api_movel(show)
    if status_movel == "disponivel":
        print(f"     API movel: DISPONIVEL!")
        resetar_erro_show(show["data"])
        return "disponivel", det_movel, False
    elif status_movel == "esgotado":
        print(f"     API movel: esgotado.")
        resetar_erro_show(show["data"])
        return "esgotado", [], False

    # ── 3. Fallback: raspagem de HTML ─────────────────────────
    for tentativa in range(tentativas):
        try:
            if tentativa > 0:
                # Delay curto entre tentativas — ScraperAPI ja resolve o 403
            ...
            # Tentativa 0: ScraperAPI premium (evita 403 direto)
            # Tentativa 1: direto mobile (fallback sem gastar credito)
            # Tentativa 2+: ScraperAPI novamente com sessao diferente
            if tentativa == 0:
                usar_scraper  = bool(SCRAPERAPI_KEY)
                usar_mobile   = False
            elif tentativa == 1:
                usar_scraper  = False
                usar_mobile   = True
            ...
            r          = fazer_request(show["url"], timeout=45, forcar_scraper=usar_scraper, usar_mobile=usar_mobile)
            html       = r.text
            html_lower = html.lower()
            ...
            if len(html) > 500_000:
                print(f" | ⚠️ pagina muito grande ({len(html)//1024}KB) — ScraperAPI com JS pesado", end="")
            ...
            # Fila virtual / captcha — status proprio para o loop principal
            sinais_fila = [
            ...
            if any(p in html_lower for p in sinais_fila):
                print(f"     Fila/captcha detectado — aguardando disponibilidade...")
                resetar_erro_show(show["data"])
            ...
            # ── Quando ha sinal de compra E esgotado ao mesmo tempo:
            # pode ser reposicao (site ainda tem texto antigo).
            # Nesse caso, confia no sinal de compra.
            if disponivel and esgotado:
                print(f"     Sinais mistos (compra+esgotado) — priorizando disponivel.")
                esgotado = False

            if disponivel:
                # ── Passo 1: fingerprint identico ao anterior? ────────
                # Se a pagina nao mudou absolutamente nada desde a ultima
                # leitura, nao houve evento real — ignora sem confirmacao.
                fp_atual    = _extrair_fingerprint_status(html)
                fp_anterior = _fingerprints_confirmados.get(show["data"], "")
                ja_tinha_hash = show["data"] in _html_hashes

                if ja_tinha_hash and fp_atual == fp_anterior:
                    print(f"     Pagina identica a leitura anterior — nenhuma mudanca real.")
                    return "nao_disponivel", [], mudou_zona

                # ── Passo 2: confirmacao unica em 15s ─────────────────
                # Fingerprint mudou → ha algo novo. Aguarda 15s e le
                # novamente. Se as duas leituras concordam: e real.
                print(f"     Mudanca detectada! Confirmando em 15s...")
                time.sleep(random.uniform(13, 17))
                try:
                    r2     = fazer_request(show["url"], timeout=45,
                                           forcar_scraper=bool(SCRAPERAPI_KEY))
                    html2  = r2.text
                    html2l = html2.lower()
                    disp2  = any(p in html2l for p in palavras_disponivel)
                    esg2   = any(p in html2l for p in palavras_esgotado)
                    if esg2 and disp2:
                        esg2 = False  # sinais mistos na confirmacao: confia no disponivel
                    print(f"     Confirmacao: disponivel={disp2} esgotado={esg2} | HTTP {r2.status_code}")
                    if not disp2:
                        print(f"     Confirmacao negativa — falso positivo descartado.")
                        return "nao_disponivel", [], mudou_zona

                    # ── Para shows com filtro (The Rose: pista premium + meia):
                    # exige que os dois apareçam juntos na 2a leitura tambem.
                    filtro_setor = show.get("setor_filtro")
                    filtro_tipo  = show.get("tipo_filtro")
                    if filtro_setor and filtro_tipo:
                        setor_str = filtro_setor.lower()
                        tipo_strs = [t.lower() for t in filtro_tipo]
                        if not (setor_str in html2l and any(t in html2l for t in tipo_strs)):
                            print(f"     Confirmacao: filtro setor/tipo nao satisfeito — ignorando.")
                            return "nao_disponivel", [], mudou_zona
                        # Verifica proximidade (janela de 600 chars)
                        prox_ok = False
                        pos = html2l.find(setor_str)
                        while pos != -1:
                            trecho = html2l[max(0, pos - 600): pos + 600]
                            if any(t in trecho for t in tipo_strs):
                                prox_ok = True
                                break
                            pos = html2l.find(setor_str, pos + 1)
                        if not prox_ok:
                            print(f"     Confirmacao: pista+meia em secoes separadas — ignorando.")
                            return "nao_disponivel", [], mudou_zona
                        print(f"     Filtro OK na confirmacao: pista premium + meia juntos.")

                    detalhes = extrair_detalhes(html2)
                    print(f"     CONFIRMADO! Disponivel nas duas leituras.")
                except Exception as e:
                    print(f"     Confirmacao falhou ({e}) — usando 1a leitura.")
                    detalhes = extrair_detalhes(html)

                _fingerprints_confirmados[show["data"]] = fp_atual
...
def monitorar():
    ...
    while True:
        try:
            ...
            if horas_desde_12h >= 12:
                ...
                for s in SHOWS:
                    ...
                    if st in ("nao_disponivel", "nao_abriu", "aguardando"):
                        ...
                        emoji = "🔴"
                        st_txt = "Esgotado — monitorando REPOSICAO 24h"
                    elif st == "fila":
                        emoji = "🟡"
                        st_txt = "Fila virtual"
                    else:
                ...
                _, frase = random.choice(FRASES_BTS + [(None, f) for f in FRASES_THE_ROSE])

                # Sorteia musica — mistura BTS e The Rose
                todas_musicas = [("BTS", n, d) for n, d in MUSICAS_BTS] + [("The Rose", n, d) for n, d in MUSICAS_THE_ROSE]
                artista_musica, nome_musica, desc_musica = random.choice(todas_musicas)

                enviar_telegram(
                    f"💜 BOT RODANDO — {horas_rodando:.0f}h SEM PARAR!\n"
                    f"🕐 {agora.strftime('%d/%m/%Y %H:%M')}\n"
                    f"🔍 Verificacoes feitas: {total_verif}\n\n"
                    f"📋 STATUS DOS 4 SHOWS:\n"
                    f"{status_txt}\n\n"
                    f"🎵 MUSICA RECOMENDADA:\n"
                    f"  {artista_musica} — {nome_musica}\n"
                    f"  {desc_musica}\n\n"
                    f"💬 FRASE DE APOIO:\n"
                    f"  {frase}\n\n"
                    f"⏱ Monitorando 24h, ~2min entre verificacoes.\n"
                    f"So aviso quando for real. Pode confiar!"
                )
                ultima_msg_12h = agora
                print("Mensagem de 12h enviada!")

            # ── Verificacao PARALELA dos shows ─────────────
            houve_mudanca_zona = False
            resultados = {}  # data -> (status, detalhes, mudou_zona)
            lock_resultados = threading.Lock()

            def _verificar_um(show_fixo, resultados_dict, lock):
                """Recebe show_fixo por argumento (evita bug de closure do Python)."""
                data    = show_fixo["data"]
                artista = show_fixo.get("artista", "Show")
                print(f"\n[{data}] Verificando {artista} - {show_fixo['dia']}...")
                try:
                    st, det, mz = verificar_show(show_fixo)
                except Exception as e:
                    print(f"   Erro na thread de {data}: {e}")
                    st, det, mz = "erro", [], False
                with lock:
                    resultados_dict[data] = (st, det, mz)

            threads = []
            for show in SHOWS:
                t = threading.Thread(
                    target=_verificar_um,
                    args=(show, resultados, lock_resultados),
                    daemon=True,
                )
                threads.append(t)
                t.start()
            ...
            # Aguarda todas as threads terminarem (timeout de 3 min por show)
            for t in threads:
                t.join(timeout=180)
            ...
            for show in SHOWS:
                ...
                if data not in resultados:
                    print(f"   [{data}] Thread nao retornou resultado — pulando.")
                    continue

                status, detalhes, mudou_zona = resultados[data]

                # Modo rapido quando zona de compra muda
                if mudou_zona and status not in ("disponivel", "fila"):
                    houve_mudanca_zona = True
                    rodadas_rapidas[data] = 5
                    print(f"   Zona mudou — modo rapido ativado.")

                anterior = status_anterior.get(data)
                status_anterior[data] = status

                # "disponivel" que chega aqui ja passou pela confirmacao
                # de 15s dentro de verificar_show — e real, sem barreiras.
                # Nao ha mais camadas de anti-spam para disponivel.

                # Cooldown: evita spam de alertas repetidos para o mesmo show
                if not hasattr(monitorar, '_ultimo_alerta'):
                    monitorar._ultimo_alerta = {}
                ultimo_alerta_show = monitorar._ultimo_alerta.get(data, 0)
                ...
                if status == "disponivel":
                    ...
                    precos    = det.get("precos", [])
                    html_raw  = det.get("_html_raw", "").lower()
                    eh_reposicao = anterior in ("esgotado", "nao_disponivel")

                    # ── Filtro de setor + tipo (The Rose) ───────────────
                    # Regra: so avisa se AMBOS aparecerem juntos na pagina.
                    # "Juntos" = dentro de uma janela de 600 caracteres
                    # no HTML, para evitar falso positivo onde "pista premium"
                    # e "meia estudante" aparecem em secoes totalmente diferentes
                    # (ex: descricao do evento + rodape de termos).
                    filtro_setor = show.get("setor_filtro")
                    filtro_tipo  = show.get("tipo_filtro")

                    if filtro_setor or filtro_tipo:
                        setor_str = filtro_setor.lower() if filtro_setor else None
                        tipo_strs = [t.lower() for t in (filtro_tipo or [])]

                        # Verifica presenca basica de cada um
                        setor_presente = (not setor_str) or (setor_str in html_raw)
                        tipo_presente  = (not tipo_strs) or any(t in html_raw for t in tipo_strs)

                        if not setor_presente:
                            print(f"   '{filtro_setor}' nao encontrado — aguardando.")
                            continue
                        if not tipo_presente:
                            print(f"   Meia/estudante nao encontrada — aguardando.")
                            continue

                        # Verifica proximidade: os dois devem aparecer dentro
                        # de uma janela de 600 chars no HTML (mesma secao)
                        if setor_str and tipo_strs:
                            janela = 600
                            proximidade_ok = False
                            idx_setor = html_raw.find(setor_str)
                            while idx_setor != -1:
                                trecho = html_raw[max(0, idx_setor - janela): idx_setor + janela]
                                if any(t in trecho for t in tipo_strs):
                                    proximidade_ok = True
                                    break
                                idx_setor = html_raw.find(setor_str, idx_setor + 1)
                            if not proximidade_ok:
                                print(f"   '{filtro_setor}' e meia encontrados mas em secoes diferentes — falso positivo ignorado.")
                                continue
                        print(f"   Filtro OK: pista premium + meia estudante confirmados juntos.")

                    # ── Monta textos completos ───────────────────────────
                    setores_txt  = "\n".join([f"  • {s}" for s in setores]) if setores else "  • Verificar no site"
                    precos_txt   = "  " + " | ".join(precos) if precos else "  Verificar no site"
                    tipos_txt    = "  " + " | ".join(tipos_raw) if tipos_raw else "  Verificar no site"
                    tag_reposicao = "\n🔄 REPOSICAO! Ingresso que estava esgotado voltou!\n" if eh_reposicao else ""

                    # ── Alerta BTS ───────────────────────────────────────
                    if artista == "BTS":
                        alerta = (
                            f"🚨 BTS - INGRESSO DISPONIVEL!{tag_reposicao}\n"
                            f"💜 BTS WORLD TOUR ARIRANG — SAO PAULO\n"
                            f"📅 {show['dia'].upper()} — {data}\n"
                            f"🕐 Detectado: {agora.strftime('%d/%m/%Y %H:%M:%S')}\n\n"
                            f"🏟 SETORES DISPONIVEIS:\n{setores_txt}\n\n"
                            f"💰 PRECOS:\n{precos_txt}\n\n"
                            f"🎟 TIPOS DE INGRESSO:\n{tipos_txt}\n\n"
                            f"🔗 LINK DIRETO:\n{show['url']}\n\n"
                            f"⚡ CORRE! ARMY FIGHTING! 💜\n"
                            f"🛒 Ja abre o site e deixa o cartao na mao!"
                        )

                    # ── Alerta The Rose ──────────────────────────────────
                    else:
                        # Detecta meia estudante especificamente
                        meia_encontrada = ""
                        for tp in (filtro_tipo or []):
                            if tp.lower() in html_raw or any(tp.lower() in t.lower() for t in tipos_raw):
                                meia_encontrada = tp.upper()
                                break

                        meia_txt = f"\n⭐ {meia_encontrada} DETECTADA!\n" if meia_encontrada else ""

                        # Tenta extrair preco especifico da pista premium meia
                        preco_pista = ""
                        for p in precos:
                            valor = p.replace("R$", "").replace(".", "").replace(",", ".").strip()
                            try:
                                v = float(valor)
                                if v < 1000:  # preco de meia costuma ser menor
                                    preco_pista = f"\n💸 Preco aproximado meia: {p}"
                                    break
                            except Exception:
                                pass

                        alerta = (
                            f"🚨 THE ROSE — PISTA PREMIUM MEIA DISPONIVEL!{tag_reposicao}\n"
                            f"🌹 THE ROSE TOUR 2026 [ROSETOPIA] — SAO PAULO\n"
                            f"📅 {show['dia'].upper()} — {data}\n"
                            f"🕐 Detectado: {agora.strftime('%d/%m/%Y %H:%M:%S')}\n\n"
                            f"✅ PISTA PREMIUM: ABERTA!{meia_txt}{preco_pista}\n\n"
                            f"🏟 SETORES DISPONIVEIS:\n{setores_txt}\n\n"
                            f"💰 TODOS OS PRECOS:\n{precos_txt}\n\n"
                            f"🎟 TIPOS DE INGRESSO:\n{tipos_txt}\n\n"
                            f"🔗 LINK DIRETO:\n{show['url']}\n\n"
                            f"⚡ CORRE! NAO PERDE ESSA CHANCE!\n"
                            f"🛒 Ja abre o site e deixa o CPF na mao!"
                        )

                    # ── Verifica cooldown antes de enviar (15 min) ───────
                    agora_ts = time.time()
                    cooldown_segundos = 900  # 15 minutos
                    if agora_ts - ultimo_alerta_show < cooldown_segundos and not eh_reposicao:
                        restante = int(cooldown_segundos - (agora_ts - ultimo_alerta_show))
                        print(f"   Disponivel mas cooldown ativo ({restante}s restantes) — sem spam.")
                        continue
                    ...
                    monitorar._ultimo_alerta[data] = agora_ts
                    print(f"   DISPONIVEL! Alerta 3x enviado! Todos os 4 shows continuam monitorados 24h.")
                    ...
                    erros_consecutivos = 0
                    # Fila virtual detectada — sem alerta (usuario pediu silencio)
                    print(f"   Fila virtual detectada — aguardando disponivel...")

                elif status == "esgotado":
                    erros_consecutivos = 0
                    # Mantém modo rapido ativo enquanto esgotado para nao
                    # perder reposicoes — intervalo de ~30s em vez de ~2min
                    rodadas_rapidas[data] = max(rodadas_rapidas.get(data, 0), 3)
                    print(f"   Esgotado - modo rapido ativo, monitorando reposicao...")
                    ...
                    if erros_consecutivos >= 5:
                        enviar_telegram(
                            f"⚠️ {erros_consecutivos} erros seguidos no BTS!\n"
                            f"Ticketmaster pode estar bloqueando.\n"
                            f"Continuando tentativas com ScraperAPI...\n"
