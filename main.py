...
def monitorar():
    ...
    # ── MENSAGEM INICIAL ──────────────────────────────────────
    msg_inicio = (
        f"✅ BOT INICIADO E MONITORANDO!\n"
        f"🕐 Inicio: {hora_inicio.strftime('%d/%m/%Y as %H:%M:%S')}\n\n"
        f"🎵 THE ROSE\n"
        f"📅 21/06/2026 - Domingo\n"
        f"🎟 Alerta: Pista Premium + Meia Estudante\n\n"
        f"💜 BTS WORLD TOUR - ARIRANG\n"
        f"📅 28/10/2026 - Quarta-feira\n"
        f"📅 30/10/2026 - Sexta-feira\n"
        f"📅 31/10/2026 - Sabado\n"
        f"🎟 Alerta: qualquer ingresso disponivel\n\n"
        f"⏱ Verificando a cada ~2 minutos, 24h por dia.\n"
        f"So aviso quando tiver disponivel de verdade!\n"
        f"A cada 12h mando confirmacao que estou rodando.\n"
        f"ARMY FIGHTING! 💜"
    )
        ...
        try:
            ...
            for show in SHOWS:
                ...
                if status == "disponivel":
                    ...
                    if artista == "BTS":
                        alerta = (
                            f"🚨🚨🚨 BTS - INGRESSO DISPONIVEL! 🚨🚨🚨\n\n"
                            f"💜 BTS WORLD TOUR - ARIRANG\n"
                            f"📅 {show['dia'].upper()} - {data}\n"
                            f"🕐 Detectado: {agora.strftime('%d/%m/%Y as %H:%M:%S')}\n\n"
                            f"✅ DISPONIVEL PARA COMPRA AGORA!\n\n"
                            f"🏟 SETORES:\n{setores_txt}\n\n"
                            f"💰 PRECOS:\n{precos_txt}\n\n"
                            f"🎟 TIPOS DE INGRESSO:\n{tipos_txt}\n\n"
                            f"🔗 LINK PARA COMPRAR:\n"
                            f"{show['url']}\n\n"
                            f"⚡ CORRE! Ingressos esgotam em minutos!\n"
                    ...
                    for conf in range(5):
                        ...
                        if st2 == "disponivel":
                            enviar_telegram(
                                f"✅ AINDA ABERTO! Confirmacao {conf+1}/5\n"
                                f"{artista} - {show['dia']} {data}\n"
                                f"🔗 {show['url']}"
                            )
                        else:
